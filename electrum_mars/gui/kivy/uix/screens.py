...
from electrum_mars.invoices import (PR_DEFAULT_EXPIRATION_WHEN_CREATING,
                                   PR_PAID, PR_UNKNOWN, PR_EXPIRED, PR_INFLIGHT,
                                   pr_expiration_values, Invoice)
from electrum_mars import bitcoin, constants
from electrum_mars import lnutil
from electrum_mars.transaction import tx_from_any, PartialTxOutput
from electrum_mars.util import (parse_URI, InvalidBitcoinURI, TxMinedInfo, maybe_extract_lightning_payment_identifier,
...
from electrum_mars.lnurl import decode_lnurl, request_lnurl, callback_lnurl, LNURLError, LNURL6Data
...
from electrum_mars.gui.kivy import KIVY_GUI_PATH
from electrum_mars.gui.kivy.i18n import _

if TYPE_CHECKING:
    from electrum_mars.gui.kivy.main_window import ElectrumWindow
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Marscoin address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid Marscoin Address') + ':\n' + address)
                        return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_mars.util import create_bip21_uri
        amount = self.app.get_amount(self.amount)
