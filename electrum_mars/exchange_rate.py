...
class Bit2C(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bit2c.co.il', '/Exchanges/MARSNIS/Ticker.json')
...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("MARS", ""), Decimal(json[r]['last']))
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['MARS'][r]) for r in json['MARS']
                 if json['MARS'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=MARS")
        return json[ccy +'_MARS']


class Bitfinex(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.bitfinex.com', '/v1/pubticker/marsusd')
...
class Bitso(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.bitso.com', '/v3/ticker/?book=mars_mxn')
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json('www.bitstamp.net', f'/api/v2/ticker/mars{ccy.lower()}/')
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=MARS')
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/marscoin/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/marscoin/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/marscoin?localization=false&tickers=false&community_data=false&developer_data=false&sparkline=false')
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/marscoin/market_chart?vs_currency=%s&days=max' % ccy)
...
class CoinSpot(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['mars']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        mars_prices = json['prices']['MARS']
        return dict([(r, Decimal(mars_prices[r])) for r in mars_prices])
...
class HitBTC(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ccys = ['USD']
        json = await self.get_json('api.hitbtc.com', '/api/2/public/ticker/MARS%s' % ccy)
...
class Kraken(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('XMARSZ')]
        json = await self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['XMARSZ'+ccy]['c'][0])
        return result
    ...
    async def request_history(self, ccy):
        query = '/0/public/OHLC?pair=MARS%s&interval=1440' % ccy
        json = await self.get_json('api.kraken.com', query)
        history = json['result']['XMARSZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=mars_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('www.mercadobitcoin.net', '/api/mars/ticker/')
...
class QuadrigaCX(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.quadrigacx.com', '/v2/ticker?book=mars_cad')
...
class Bitragem(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.bitragem.com', '/v1/index?asset=MARS&market=BRL')
