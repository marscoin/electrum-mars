...
from electrum_mars.util import create_and_start_event_loop
from electrum_mars.commands import Commands, eval_bool
from electrum_mars import storage, wallet
from electrum_mars.wallet import restore_wallet_from_text
from electrum_mars.address_synchronizer import TX_HEIGHT_UNCONFIRMED
from electrum_mars.simple_config import SimpleConfig
from electrum_mars.transaction import Transaction, TxOutput, tx_from_any
...
class TestCommands(ElectrumTestCase):
    ...
    def test_export_private_key_imported(self, mock_save_db):
        ...
        with self.assertRaises(Exception):
            cmds._run('getprivatekeys', ("mars1qgfam82qk7uwh5j2xxmcd8cmklpe0zackqwq8jp",), wallet=wallet)  # not in wallet
        self.assertEqual("p2wpkh:TAa25Tq4PdzhDKBoVaFaCdV3yxvLrRikQviNkuFQLeYopsVvNTV3",
                         cmds._run('getprivatekeys', ("mars1q2ccr34wzep58d4239tl3x3734ttle92arvely7",), wallet=wallet))
        # list of addresses tests
        with self.assertRaises(Exception):
            cmds._run('getprivatekeys', (['mars1q2ccr34wzep58d4239tl3x3734ttle92arvely7', 'asd'],), wallet=wallet)
        self.assertEqual(['p2wpkh:TAa25Tq4PdzhDKBoVaFaCdV3yxvLrRikQviNkuFQLeYopsVvNTV3', 'p2wpkh:TAgoypi14k5Y54svysG62xp5QFRWiF1W64zxaFRFPo2jMPSMoa5D'],
                         cmds._run('getprivatekeys', (['mars1q2ccr34wzep58d4239tl3x3734ttle92arvely7', 'mars1q9pzjpjq4nqx5ycnywekcmycqz0wjp2nq7n0qjr'],), wallet=wallet))
    ...
    def test_export_private_key_deterministic(self, mock_save_db):
        ...
        self.assertEqual("p2wpkh:T6v5Q8KEmjLmJoTxPfXfyNcCEFYC7Lfmwmp9Y8dce9knevo9ZkPk",
                         cmds._run('getprivatekeys', ("mars1q3g5tmkmlvxryhh843v4dz026avatc0zz8xd49e",), wallet=wallet))
        # list of addresses tests
        with self.assertRaises(Exception):
            cmds._run('getprivatekeys', (['mars1q3g5tmkmlvxryhh843v4dz026avatc0zz8xd49e', 'asd'],), wallet=wallet)
        self.assertEqual(['p2wpkh:T6v5Q8KEmjLmJoTxPfXfyNcCEFYC7Lfmwmp9Y8dce9knevo9ZkPk', 'p2wpkh:TAgoypi14k5Y54svysG62xp5QFRWiF1W64zxaFRFPo2jMPSMoa5D'],
                         cmds._run('getprivatekeys', (['mars1q3g5tmkmlvxryhh843v4dz026avatc0zz8xd49e', 'mars1q9pzjpjq4nqx5ycnywekcmycqz0wjp2nq7n0qjr'],), wallet=wallet))
...
class TestCommandsTestnet(TestCaseForTestnet):
    ...
    def test_serialize(self):
        ...
        jsontx = {
            ...
            "outputs": [
                {
                    "address": "tmars1q4s8z6g5jqzllkgt8a4har94wl8tg0k9m77w2jy",
                    "value_sats": 990000
                }
            ]
        }
    ...
    def test_payto(self, mock_save_db):
        ...
        tx_str = cmds._run(
            'payto', (),
            destination="tmars1qsyzgpwa0vg2940u5t6l97etuvedr5dejcp84ad",
            amount="0.00123456",
            feerate=50,
            locktime=1972344,
            wallet=wallet)
        ...
        txout = TxOutput.from_address_and_value("tmars1qsyzgpwa0vg2940u5t6l97etuvedr5dejcp84ad", 123456)
