...
```
$ adb -d install -r dist/Electrum-MARS-*-arm64-v8a-debug.apk
$ adb shell monkey -p org.electrum_mars.electrum_mars 1
```
...
```
adb logcat | grep -F "`adb shell ps | grep org.electrum_mars.electrum_mars | cut -c14-19`"
```
...
Run electrum with the `-g` switch: `electrum-mars -g kivy`
...
```
$ adb shell
$ run-as org.electrum_mars.electrum_mars ls /data/data/org.electrum_mars.electrum_mars/files/data
$ run-as org.electrum_mars.electrum_mars cp /data/data/org.electrum_mars.electrum_mars/files/data/wallets/my_wallet /sdcard/some_path/my_wallet
```
