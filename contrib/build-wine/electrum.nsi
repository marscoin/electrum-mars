...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-MARS"
  !define PRODUCT_WEB_SITE "https://github.com/marscoin/electrum-mars"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-mars-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-mars\electrum_mars\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-mars\*.*"
  File "c:\electrum-mars\electrum_mars\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mars-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-mars-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-mars-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-mars-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-mars-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\marscoin" "" "URL:marscoin Protocol"
  WriteRegStr HKCU "Software\Classes\marscoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\marscoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\marscoin\shell\open\command" "" "$\"$INSTDIR\electrum-mars-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\marscoin"
